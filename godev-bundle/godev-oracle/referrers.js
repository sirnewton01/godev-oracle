...
function(mBootstrap, xhr) {
	...
	mBootstrap.startup().then(function(core) {
		...
		var runQuery = function(evt) {
			...
			document.cookie = "referrers.scope=" + cmd + "; path=/";
			
			xhr("GET", "/go/bundle-cgi/godev-oracle/referrers/" + resource + "?pos=" + selection + "&scope=" + cmd,
			{
				headers: {},
				timeout: 60000
				...
				if (result.response !== "" && result.response !== '""') {
					...
					if (impl) {
						...
						for (var idx = 0; idx < hits.length; idx++) {
							...
							entry.innerHTML = impl.logicalRefs[idx];
							
							pos = impl.logicalRefs[idx];
							file = pos.split(":")[0];
							line = pos.split(":")[1];
		...
		xhr("GET", "/debug/commands",
			{
				...
				var beginIdx = document.cookie.indexOf("referrers.scope");
				if (beginIdx !== -1) {
					...
					var cmd = document.cookie.substring(beginIdx + 16, endIdx);
