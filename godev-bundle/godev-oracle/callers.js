...
function(mBootstrap, xhr) {
	...
	mBootstrap.startup().then(function(core) {
		...
		var runQuery = function(evt) {
			...
			document.cookie = "callers.scope=" + cmd + "; path=/";
			
			xhr("GET", "/go/bundle-cgi/godev-oracle/callers/" + resource + "?pos=" + selection + "&scope=" + cmd,
			{
				headers: {},
				timeout: 60000
				...
				if (result.response !== "" && result.response !== '""') {
					...
					if (impl) {
						...
						for (var idx = 0; idx < hits.length; idx++) {
							...
							// This is some kind of non-resolvable caller
							//   such as a "synthetic call".
							if (hits[idx].pos === "" && hits[idx].logicalPos === "") {
								cell.innerHTML = hits[idx].desc;
								continue;
							}
							
							entry = document.createElement("a");
							entry.innerHTML = hits[idx].logicalPos;
		...
		xhr("GET", "/debug/commands",
			{
				...
				var beginIdx = document.cookie.indexOf("callers.scope");
				if (beginIdx !== -1) {
					...
					var cmd = document.cookie.substring(beginIdx + 14, endIdx);
